...
def is_first_of_the_month(day_number):
  ...
  #pass
  if day_number == 1: 
    return "First of the month!" #no need for print, just do what is asked.
  else: #needs to be lined up to work
...
def has_five_chars(the_str):
  ...
  #pass
  if len(the_str) == 5:
    return f"{the_str} is five characters long" #trick question with STRING -> f"{the_str}"
  else:
